...
for train_file in train_files:
    # Extract clean dataset name (e.g., "abc" from "/content/abc_train.csv")
    base_name = os.path.basename(train_file).replace("_train.csv", "")
    test_file = train_file.replace("_train.csv", "_test.csv")   # construct test file path
    ...
    plt.title(f"{base_name} Transformer Loss")          # Use clean name
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)

    path = f"plots/{base_name}_transformer_loss.png"    # Use clean name
    plt.savefig(path)
    plt.close()

    print(f"📊 Saved: {path}")

    results.append((base_name, avg_test))
