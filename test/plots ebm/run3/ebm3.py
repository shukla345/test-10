...
def train_ebm(model, dataloader, mask_idx, epochs, lr=1e-4, device='cuda', print_every=10):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # --- MODIFICATION: Multi-step Discrete Sampler ---
            mask = (empty == mask_idx)
            neg = solved.clone()
            vocab_size = model.embed.num_embeddings
            
            # 1. Start completely random in the masked regions (explore deep space)
            if mask.any():
                neg[mask] = torch.randint(2, vocab_size, (mask.sum().item(),), device=device)
                
                # 2. Take K steps to fall into energy holes (Discrete MCMC)
                num_steps = 20  # Increase if holes persist, but slows training
                masked_indices = torch.nonzero(mask, as_tuple=True)
                
                for _ in range(num_steps):
                    proposal = neg.clone()
                    
                    # Propose changing a random 30% of the masked cells
                    noise_mask = torch.rand(len(masked_indices[0]), device=device) < 0.3
                    if noise_mask.any():
                        new_tokens = torch.randint(2, vocab_size, (noise_mask.sum().item(),), device=device)
                        proposal[masked_indices[0][noise_mask], masked_indices[1][noise_mask], masked_indices[2][noise_mask]] = new_tokens
                        
                        # Evaluate energy of current state vs proposal
                        with torch.no_grad():
                            e_current = model(empty, neg).view(-1)
                            e_proposal = model(empty, proposal).view(-1)
                        
                        # Accept the proposal if it Lowers the energy (Greedy sampling)
                        accept = e_proposal < e_current
                        accept_expanded = accept.view(-1, 1, 1).expand_as(neg)
                        neg = torch.where(accept_expanded, proposal, neg)
            # -------------------------------------------------  #
            energy_fake = model(empty, neg).mean()
            # --- MODIFICATION: L2 Regularization on Energy ---
            alpha = 0.1  # Regularization strength. Tweak between 0.01 and 1.0
            l2_penalty = (energy_real**2 + energy_fake**2)
            loss = (energy_real - energy_fake) + (alpha * l2_penalty)
...
def process_dataset(prefix, args, device):
    ...
    # File paths
    empty_file = os.path.join(args.data_dir,
                          f"{prefix}_checkerboard_empty.json")

    solved_file = os.path.join(args.data_dir,
                               f"{prefix}_checkerboard_solved.json")
    
    vocab_file = os.path.join(args.data_dir,
                              f"{prefix}_vocab.json")
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="/kaggle/input/datasets/classstudents/energy04/", help="Directory containing dataset files")
    parser.add_argument("--plot_dir", type=str, default="/kaggle/working/plots", help="Root directory to save plots")
