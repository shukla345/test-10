...
BATCH_SIZE = 128
LR         = 1e-3
HIDDEN_DIM = 128      # initial projection dim
GROWTH_RATE= 32      # how much each layer adds
NUM_LAYERS = 8
DROPOUT    = 0
WD         = 0
EPOCHS     = 200
...
class DenseLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat([x, new_features], dim=1)  # 🔥 key DenseNet idea
...
class DenseBlock(nn.Module):
    def __init__(self, in_dim, num_layers, growth_rate, dropout):
        ...
        for _ in range(num_layers):
            ...
            current_dim += growth_rate  # grows after each layer
...
for train_file in train_files:
    ...
    model = DenseMLP(in_dim, HIDDEN_DIM, NUM_LAYERS, GROWTH_RATE, out_dim, DROPOUT).to(device)
